...
const categoryClass = computed(() => CATEGORY_CLASSES[props.event.category] || 'tag-lavender')
...
.event-card:hover { border-color: var(--border-hover); transform: translateY(-4px); box-shadow: 0 24px 48px rgba(0,0,0,0.3); }
...
.card-category { position: absolute; top: 14px; left: 14px; }
.card-ongoing {
  position: absolute;
  bottom: 14px;
  left: 14px;
...
.card-city { font-size: 13px; font-weight: 600; color: var(--lavender); letter-spacing: 0.03em; text-transform: uppercase; }
.card-dot { width: 3px; height: 3px; background: var(--text-muted); border-radius: 50%; }
.card-date { font-size: 13px; color: var(--text-muted); }
.card-title { font-family: var(--font-display); font-size: 17px; font-weight: 600; line-height: 1.3; margin-bottom: 16px; color: var(--text); }
.card-footer { display: flex; justify-content: space-between; align-items: center; }
.card-host { font-size: 13px; color: var(--text-muted); }
.card-duration { font-size: 12px; color: var(--text-muted); background: var(--bg-3); padding: 4px 10px; border-radius: 100px; }
.card-hover-overlay { position: absolute; inset: 0; background: rgba(36,49,107,0.7); display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity var(--transition); }
.event-card:hover .card-hover-overlay { opacity: 1; }
.card-cta { background: var(--yellow); color: var(--bg); padding: 12px 28px; border-radius: var(--radius); font-size: 14px; font-weight: 600; }
</style>